...
const Face = () => {
  ...
  useEffect(() => {
    ...
    const detectMood = async () => {
      ...
      if (detections.length > 0) {
        const topMood = Object.entries(detections[0].expressions)
  ...
  const fetchSongsByMood = async (mood) => {
    try {
      const res = await axios.post(`${url}/song/by-mood`, { mood });
  ...
  // Handle play to pause other audios
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      {!snapshot ? (
        ...
        <div className="flex flex-col items-center">
          <img
            ...
            className="rounded shadow-md"
          />
          <p className="text-white mt-2 text-sm">📸 Snapshot captured!</p>
          <button
      ...
      <p className="mt-2 text-lg text-white">
        Detected Mood: <strong className="capitalize text-blue-400">{mood}</strong>
      </p>

      <div className="mt-4 w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {songs.length > 0 ? (
          songs.map((song) => (
            <div key={song._id} className="bg-gray-800 p-4 rounded shadow-md text-white">
              <img
                src={song.image}
                alt={song.name}
              ...
              <p className="text-sm mb-2">{song.album}</p>
              <audio
                controls
                src={song.file}
                className="w-full"
          ...
          <p className="text-white">No songs found for mood: {mood}</p>
        )}
      </div>
    </div>
  );
